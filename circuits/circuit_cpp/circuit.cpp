...
void checkTriangle_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void checkTriangle_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 161;}

uint get_number_of_components() {return 6;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 142;}
...
ctx->componentMemory[coffset].templateName = "checkTriangle";
ctx->componentMemory[coffset].signalStart = soffset;
...
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 4],&signalValues[mySignalStart + 6]); // line circom 20
Fr_mul(&expaux[0],&signalValues[mySignalStart + 1],&expaux[2]); // line circom 20
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 6],&signalValues[mySignalStart + 2]); // line circom 21
Fr_mul(&expaux[0],&signalValues[mySignalStart + 3],&expaux[2]); // line circom 21
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 4]); // line circom 22
Fr_mul(&expaux[0],&signalValues[mySignalStart + 5],&expaux[2]); // line circom 22
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_add(&expaux[1],&signalValues[mySignalStart + 7],&signalValues[mySignalStart + 8]); // line circom 24
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 9]); // line circom 24
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&circuitConstants[0]); // line circom 28
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 28. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
{
PFrElement aux_dest = &lvar[0];
// load src
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 3]); // line circom 41
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 4]); // line circom 42
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 5]); // line circom 43
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 4],&signalValues[mySignalStart + 6]); // line circom 44
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 11],&signalValues[mySignalStart + 11]); // line circom 56
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 12],&signalValues[mySignalStart + 12]); // line circom 57
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 13],&signalValues[mySignalStart + 13]); // line circom 58
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 14],&signalValues[mySignalStart + 14]); // line circom 59
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 15],&signalValues[mySignalStart + 16]); // line circom 62
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0],&circuitConstants[1]); // line circom 64
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 64. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
{
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 17],&signalValues[mySignalStart + 18]); // line circom 66
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// no need to run sub component
...
checkTriangle_3_create(1,0,ctx,"main",0);
checkTriangle_3_run(0,ctx);
}
